...
- **DB 스냅샷:** `python snapshot_db.py` (지금 상태를 `snapshots/`에 CSV로 떠둡니다)
  자재 표에는 시각 기록이 없어 **재고는 이 파일이 아니면 되돌릴 근거가 없습니다.**
  되돌리는 방법은 스냅샷 폴더의 `요약.txt`에 함께 적힙니다.

파이썬 스크립트는 이 넷뿐입니다. 새로 만들기 전에 기존 것에 합칠 수 없는지 먼저 볼 것.
(이관·조사용 일회성 스크립트는 쓰고 나서 지웁니다.)
